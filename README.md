...
Можно переопределить container binding `MaxClientContract` или `MessageFormatterContract`. При разрешении контракта фабрика передаёт объединённую конфигурацию в параметре `options`. Для ручной отправки доступен `app(MaxClientContract::class)->sendMessage('...')`; эта отправка обходит форматирование и лимиты handler.
